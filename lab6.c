...
//that is, no action if  (1.1 * lightroom < light < 1.5 * lightroom).
        ...
        if(light < lightroom * 1.006 && light > lightroom * 1.002)
          {} else
          {
            if(light >= lightroom * 1.01) {P1OUT |=  BIT4; __delay_cycles(200);} // on if dark
            if(light <= lightroom * 1.002) {P1OUT &= ~BIT4; __delay_cycles(200);} // off if light
          }
